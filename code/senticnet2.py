...
for j in range(1,(len(a)+1)/10):
    name = a[j*10][0]
    if name not in senticnet.keys():
        att = []
        for i in range(1,11):
            if a[j*i][2] == 1:
                if i == 2 or i == 5 or i == 6 or i == 10:
                    continue
                #print i
                att.append(a[j+i][1])

        if a[j+5][2] == 1:
                sign.append(a[j+5][1],-1.0)
        if a[j+6][2] == 1:
                sign.append(a[j+6][1],1.0)

        swdnet[name] = att

print len(a),len(swdnet)
#for i in range(10):
